<!-- NOTE: This file should replace /mcp/SKILL.md in the hol-light repo.
     It adds ARM_VSTEPS_TAC, MONOTONE_MAYCHANGE_TAC, CONV_TAC WORD_BLAST,
     and the VSTEPS/frame-closure patterns to the s2n-bignum ARM section. -->
...
| `ARM_VSTEPS_TAC EXEC (n--m)` | Like ARM_STEPS_TAC but keeps register hypotheses alive (no CLARIFY_TAC) |
| `ARM_SIM_TAC EXEC (1--n)` | INIT + STEPS + FINAL in one shot |
| `ENSURES_FINAL_STATE_TAC` | Close postcondition and frame |
| `MONOTONE_MAYCHANGE_TAC` | Prove MAYCHANGE frame subgoals after ENSURES_FINAL_STATE_TAC |
...
| `CONV_TAC WORD_BLAST` | Bit-blasting for word goals (128-bit structural equalities) |
...
ARM_STEPS_TAC runs CLARIFY_TAC after each step, which substitutes register values into the goal and discards them. After many steps, memory read-back expressions become too large for WORD_BLAST. Use VSTEPS for a small window (2–8 steps) around a store to keep hypotheses alive:
```
ARM_VSTEPS_TAC EXEC (325--332) THEN
SUBGOAL_THEN `read (memory :> bytes128 ptr) (s332:armstate) = spec`
  ASSUME_TAC THENL
...
- VSTEPS can't handle D-register instructions (INS/MOV element) — use ARM_STEPS_TAC for those
- Only ~8 proofs in s2n-bignum use VSTEPS — it's specialized for when expressions blow up

### Frame condition closure
After `ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[]`, close MAYCHANGE goals with:
```
