#!/usr/bin/env python3
"""Mixed-workload analysis for the mix4s4 experiment.

  analyze_mixmx.py mix logs/mixmx_<host>.log     -- the 12-slot mix run
  analyze_mixmx.py aa  logs/mixaa_mx_<host>.log  -- 4-slot placement controls
                                                    and paired both-rank pairs

Mixes A-D are the same sequences as _docs/fused-t4p8.md, so the numbers are
directly comparable with that report's tables.
"""
...
def mixrun(path):
    ...
    print("| mix | base A/A | dsp0 A/A | m4s4 A/A |")
    print("|---|---:|---:|---:|")
    for m in mixes:
        print("| %s | %.2f | %.2f | %.2f |"
