...
#
# Also asserts the make-built object is byte-identical to /tmp/fsp/obj/<v>.o,
# i.e. that mk.sh reproduces the Makefile rule exactly.
set -e
SRC=$HOME/whole-proofs/s2n-bignum
T=/tmp/fst/tree
rm -rf /tmp/fst; mkdir -p $T/arm
...
for v in "$@"; do
  cp /tmp/fsp/src/$v.S $T/arm/aes-gcm/aesv8_gcm_8x_dec_256_wb.S
  ...
  m2=$(md5sum /tmp/fsp/obj/$v.o | cut -d' ' -f1)
