 ...
 * Sizes cover every nblk = 1..8 (the eight new fused entry points) plus
 * 16/32/64/256 blocks (256/512/1024/4096 B).
 */
...
static int VOIDRET[8];

#define BLK 16u
#define MAXB 300u
static uint8_t in_ct[MAXB*BLK] __attribute__((aligned(64)));
static uint8_t out[NV][MAXB*BLK] __attribute__((aligned(64)));
/* Real AES-256 schedule + real H^1..H^8 table, built with aws-lc's read-only
 * aarch64 asm (aesv8-armx.S / ghashv8-armx.S).  Needed because aws-lc's 4x
 * fallback kernel aes_gcm_dec_kernel reads key->rounds at [key+240] and uses
 * H^1..H^4, so a byte-for-byte cross-check against the 8x kernels is only
 * valid with a cryptographically consistent table. */
...
/* Byte-compare out/Xi/ivec/return-value of every variant against slot 0 over
 * EVERY whole-block length 1..256 blocks (16 B .. 4096 B). */
