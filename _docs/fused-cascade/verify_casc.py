...
LBL = re.compile(r"^(\.L256_dec_casc_[A-Za-z0-9_]+):")
...
def main(path):
    ...
    common = s(".L256_dec_casc_small") + s(".L256_dec_casc_34") + \
        s(".L256_dec_casc_hi") + s(".L256_dec_casc_78")
    epi = s(".L256_dec_casc_done")
    print("\nnblk  common  stub  sections  epi   slots  floor@4/cyc   aese")
    for n in range(1, 9):
        st = s(".L256_dec_casc_stub_%d" % n)
        sec = sum(s(".L256_dec_casc_%d" % j) for j in range(1, n + 1))
