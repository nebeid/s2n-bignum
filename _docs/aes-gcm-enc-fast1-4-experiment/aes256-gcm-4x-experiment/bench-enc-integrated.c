...
int main(int argc, char **argv) {
    ...
    static const size_t short_blocks[] = {
        1, 2, 3, 4, 5, 6, 7, 8,
    };
    static const size_t large_blocks[] = {
        84, 128, 256, 512, 1024, 2048,
    };
    const int large = getenv("LARGE_SIZES") != NULL;
    const size_t *blocks = large ? large_blocks : short_blocks;
    const size_t num_sizes = large
        ? sizeof(large_blocks) / sizeof(large_blocks[0])
        : sizeof(short_blocks) / sizeof(short_blocks[0]);
