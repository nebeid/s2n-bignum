# Archive: original AES-256-GCM small-path experiment

> **Historical report (August 2026).** This preserves the original
> full-versus-compact 8x campaign and the decisions made at that stage. For
> current 4x and 8x recommendations, read the
> [authoritative experiment report](aes256-gcm-4x-experiment/README.md).
> No kernel in this archive is presented as a current recommendation.
...
In August 2026, this report chose between the **full and compact 8x encrypt
kernels**. Its statement that compact 8x preserves `fast1`--`fast4`
performance through 64 B compares only those two historical 8x designs. The
compact kernel is now a control in the authoritative report, not the current
8x recommendation.
...
approximately 10% 128-byte gain, and left 80--112 bytes unchanged.
The then-current decrypt PR used a shared one-block cascade for 1--4 blocks
at 5,960 bytes
...
| decrypt shared 1--4 at the time | 5,960 B | 1.20x | 16--64 B | shared code across four sizes |
...
At that stage, the shared 1--4-block decrypt path was 5,960 bytes. In the PR
